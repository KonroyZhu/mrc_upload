...
class QA_PS(nn.Module):  # param: 16821760
    def __init__(self, options, embedding=None):
        super(QA_PS, self).__init__()
        ...
        self.d_encoder = nn.LSTM(input_size=embedding_size, hidden_size=encoder_size, batch_first=True,
                                 bidirectional=True,
                                 bias=False, dropout=self.drop_out)
        self.a_pos = PosEncoder(d_model=embedding_size, dropout=self.drop_out)
        self.q_pos = PosEncoder(d_model=2 * encoder_size, dropout=self.drop_out)
        self.d_pos = PosEncoder(d_model=2 * encoder_size, dropout=self.drop_out)

        self.a_self = SelfAttention(encoder_size=embedding_size, num_header=4)
        self.q_self = SelfAttention(encoder_size=2 * encoder_size, num_header=4)
        self.d_self = SelfAttention(encoder_size=2 * encoder_size, num_header=4)
    ...
    def forward(self, inputs):
        ...
        a_embedding = self.a_pos(a_embedding)  # pos emb added
        a_embedding = self.a_self(a_embedding.transpose(1, 2)).transpose(1, 2) # self emb added
        ...
        Q = self.q_pos(Q)
        Q = self.q_self(Q.transpose(1,2)).transpose(1,2)
        Q = F.dropout(Q,self.drop_out)
        D, _ = self.d_encoder(d_emb)
        D = F.dropout(D, self.drop_out)  # (b,d,2h)
        D = self.d_pos(D)
        D = self.d_self(D.transpose(1,2)).transpose(1,2)
        D = F.dropout(D,self.drop_out)
        ...
        loss = self.prediction_layer(Q, U, a_embedding, is_train=is_train, is_argmax=is_argmax,print_score=True)
        return loss
