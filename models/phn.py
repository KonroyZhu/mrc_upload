...
class PHN(nn.Module):  # param: 16821760
    def __init__(self, options, embedding=None):
        ...
        self.position_t = torch.rand(options["p_len"]).cuda()
        ...
        self.position_t2 = torch.rand(options["p_len"]).cuda()

        # if torch.cuda.is_available():
        #     self.position_t.cuda()
        #     self.position_t2.cuda()
    ...
    def forward(self, inputs,shuffle=True):
        ...
        if shuffle:
            answer,labels=ans_shuffle(answer)
        else:
            labels=None
        opts = self.opts
        # Embedding
        ...
        a_emb = a_output.view(answer.size(0), 3, a.size(2))  # (b,3,2h)
        ...
        def get_position(pos_weight):
            ...
            position_T = position_T.unsqueeze(0).unsqueeze(1).unsqueeze(3).repeat(answer.size(0), 3, 1, 1)
        ...
        # if torch.cuda.is_available():
        tk_sort = torch.cuda.FloatTensor(tk_sort_init)  # pytorch中只有float_tensor可以被优化
        # else:
        #     tk_sort = torch.FloatTensor(tk_sort_init)
